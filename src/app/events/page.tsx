...
import { deactivateEvent, Event, getEvents, eventDuration } from "@/models/Event";
import { Separator } from "@/components/ui/separator";
import { Badge } from "@/components/ui/badge";
import { format, parseISO, getMonth, getYear } from 'date-fns';
import { MapPin, Clock, Calendar, Trash, Plus, Filter, Ellipsis } from "lucide-react";
                ...
                <div className="flex flex-col gap-3 md:grid grid-cols-2 overflow-y-auto max-h-screen rounded-md">
                    {filteredEvents.map((event, index) => (
                        <div key={index} className="p-4 border rounded-lg flex flex-col gap-2 backdrop-contrast-50 backdrop-opacity-25">
                            ...
                            <div className="text-xs text-balance truncate">{event.description || "No description"}</div>
