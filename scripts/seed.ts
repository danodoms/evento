...
const main = async () => {
  ...
  const deptIds = (await pool.query('SELECT id FROM public.departments')).rows.map(row => row.id);
  console.log("Generating students...");
  const studentData = generateStudents(2000, deptIds);
  await insertData('public.students', studentData);
  console.log("Students inserted.");

  const studentIds = (await pool.query('SELECT id FROM public.students')).rows.map(row => row.id);
...
main().catch(err => console.error('Main error:', err)).finally(() => pool.end());
